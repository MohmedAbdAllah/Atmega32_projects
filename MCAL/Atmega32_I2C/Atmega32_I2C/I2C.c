...
uint8_t I2C_master_addr(uint8_t ID ,uint8_t RW){
	...
	else if(((TWSR&0xF8) == 0x20)||((TWSR&0xF8) == 0x48)) return 1;
	else return 2;
}
/**************************************************************************
...
void I2C_slave_write(uint8_t data){
	...
	TWCR=(1<<TWEN)|(1<<TWINT);
	while(READBIT(TWCR,TWINT) == 0);
	if((TWSR&0xF8)==0x28)		/* Check for data transmitted &ack received */
    	return 0;			/* Return 0 to indicate ack received */
    if((TWSR&0xF8)==0x30)		/* Check for data transmitted &nack received */
